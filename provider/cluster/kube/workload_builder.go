...
const (
	...
	StorageAttributeReadOnly   = "readOnly"
)
...
func newWorkloadBuilder(log log.Logger, settings Settings, lid mtypes.LeaseID, group *manifest.Group, service *manifest.Service) workloadBuilder {
...
func (b *workloadBuilder) container() corev1.Container {
	falseValue := false
	...
	for i, storage := range b.service.Resources.Storage {
		attr := storage.Attributes.Find(StorageAttributeMount)
		if mount, _ := attr.AsString(); mount != "" {
			kcontainer.VolumeMounts = append(kcontainer.VolumeMounts, corev1.VolumeMount{
				// fixme(troian) using index to bind to pvc name might be not clever idea
				// matches VolumeName in persistentVolumeClaims below
				// should consider further usage of config maps
				Name: fmt.Sprintf("%s%d", b.service.Name, i),
				// TODO all volumes are mounted as RW for now.
				// when configmaps become supported that behaviour should be changed
				ReadOnly:  false,
				MountPath: mount,
			})
		} else {
...
func (b *workloadBuilder) persistentVolumeClaims() []corev1.PersistentVolumeClaim {
...
func (b *workloadBuilder) addEnvVarsForDeployment(envVarsAlreadyAdded map[string]int, env []corev1.EnvVar) []corev1.EnvVar {
