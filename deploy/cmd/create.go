...
func createCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			wfb := newWaitForBids(dd, gClientDir, retryConfiguration, bids)
			group.Go(func() error {
				if err = wfb.run(clientCtx, cmd.Flags(), ctx, cancel); err != nil && !errors.Is(err, context.Canceled) {
...
func newWaitForBids(dd *DeploymentData, gClientDir *gateway.ClientDirectory, retryConfiguration []retry.Option, bids <-chan mtypes.EventBidCreated) *waitForBids {
	return &waitForBids {
		bids: bids,
...
func (wfb *waitForBids) run(clientCtx client.Context, flags *pflag.FlagSet, ctx context.Context, cancel context.CancelFunc) error {
	var bidDeadline <- chan time.Time
	allBids := make(map[uint32]bidList, 0)
	loop:
	for {
		select {
		case bid := <-wfb.bids:
	...
	for gseq, bidsForGroup := range allBids {
		...
		if len(allBids) > 1 && winningBid.Price.Equal(bidsForGroup[1].Price) {
			...
			rng := rand.New(rand.NewSource(int64(winningBid.ID.DSeq)))
